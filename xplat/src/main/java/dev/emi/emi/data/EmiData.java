	...
	public static void init(Consumer<EmiResourceReloadListener> register) {
		...
		register.accept(
			new EmiDataLoader<Map<String, EmiRecipeCategoryProperties>>(
				Identifier.of("emi:category_properties"), "category/properties", Maps::newHashMap,
				(map, json, id) -> {
						...
						if (JsonHelper.hasJsonObject(json, k)) {
							...
							if (JsonHelper.hasJsonObject(val, "icon")) {
								...
								if (JsonHelper.hasString(icon, "texture")) {
									props.icon = () -> new EmiTexture(Identifier.of(JsonHelper.getString(icon, "texture")), 0, 0, 16, 16, 16, 16, 16, 16);
							...
							if (JsonHelper.hasJsonObject(val, "simplified_icon")) {
								...
								if (JsonHelper.hasString(icon, "texture")) {
									props.simplified = () -> new EmiTexture(Identifier.of(JsonHelper.getString(icon, "texture")), 0, 0, 16, 16, 16, 16, 16, 16);
								} else if (JsonHelper.hasString(icon, "stack")) {
		...
		register.accept(
			new EmiDataLoader<List<Predicate<EmiRecipe>>>(
				Identifier.of("emi:recipe_filters"), "recipe/filters", Lists::newArrayList,
				(list, json, oid) -> {
		...
		register.accept(
			new EmiDataLoader<List<Supplier<IndexStackData>>>(
				Identifier.of("emi:index_stacks"), "index/stacks", Lists::newArrayList,
		...
		register.accept(
			new EmiDataLoader<List<Supplier<EmiAlias>>>(
				Identifier.of("emi:aliases"), "aliases", Lists::newArrayList,
				(list, json, id) -> {
		...
		register.accept(
			new EmiDataLoader<List<Supplier<EmiRecipe>>>(
				Identifier.of("emi:recipe_additions"), "recipe/additions", Lists::newArrayList,
				(list, json, oid) -> {
					String s = JsonHelper.getString(json, "type", "");
					Identifier id = Identifier.of("emi:/generated/" + oid.getPath());
